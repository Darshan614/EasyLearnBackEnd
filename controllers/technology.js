...
exports.addQuestion = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(200)
      .send({ message: "Question added", subtopic: subtopic.questions });
    // return newQuestion;
  } catch (error) {
    console.error(error);
    throw error;
...
exports.addAnswer = (req, res, next) => {
  ...
  Technology.findOneAndUpdate(
    ...
    .populate("subtopics.questions.userId", "username")
    .populate("subtopics.questions.answers.userId", "username")
    .exec((err, technology) => {
      if (err) {
        console.log(err);
      } else {
        const subtopic = technology.subtopics.find((subtopic) =>
        ...
        res
          .status(200)
          .send({ message: "Answer added", questions: subtopic.questions });
      }
    });
};
...
exports.addVideo = (req, res, next) => {
  ...
  Technology.updateOne(
    ...
    {
      $push: {
        "subtopics.$.videos": { url: req.body.video, userId: req.userId },
      },
    },
    function (err, result) {
      if (err) {
...
exports.vote = async (req, res, next) => {
  ...
  try {
    ...
    const subtopic = technology.subtopics.find((sub) =>
      sub._id.equals(SUBTOPIC_ID)
    );
    ...
    const question = subtopic.questions.find((ques) =>
      ques._id.equals(QUESTION_ID)
    );
    ...
    const hasUpvoted = answer.upvoters.some((upvoter) =>
      upvoter.equals(USER_ID)
    );
    const hasDownvoted = answer.downvoters.some((downvoter) =>
      downvoter.equals(USER_ID)
    );
    ...
    if (voteType === "upvote" && !hasUpvoted) {
      ...
      if (hasDownvoted) {
        ...
        answer.downvoters = answer.downvoters.filter(
          (downvoter) => !downvoter.equals(USER_ID)
        );
      }
      ...
      if (hasUpvoted) {
        ...
        answer.upvoters = answer.upvoters.filter(
          (upvoter) => !upvoter.equals(USER_ID)
        );
      }
    } else {
